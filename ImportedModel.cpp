...
ImportedModel::ImportedModel(const char *filePath) {
	...
	modelImporter.parseOBJ2(filePath);
	cout << "ok1" << endl;
...
void ModelImporter::parseOBJ2(const char* filePath) {
	float x, y, z;
	string content;
	...
	while (!fileStream.eof()) {
		...
		// line.compare比较 从下标0开始比较2个长度，对比对象是"v "
		if (line.compare(0, 2, "v ") == 0) {
			stringstream ss(line.erase(0, 1));	//删除字母v
			ss >> x; ss >> y; ss >> z;		//按空格分隔、分配
		...
		if (line.compare(0, 2, "f ") == 0) {
			int v[3];
			stringstream ss(line.erase(0, 2));	//删掉了"f "剩下的第一个就是数字
			ss >> v[0]; ss >> v[1]; ss >> v[2];
			

			for (int i = 0; i < 3; i++)
			{
				//下标从0开始，*3的原因是因为一个点的三个属性都放到了vertVals中
				int vertRef = (v[i] - 1) * 3;
...
void ModelImporter::parseOBJ(const char *filePath) {
	...
	while (!fileStream.eof()) {
		...
		if (line.compare(0, 2, "f ") == 0) {
			...
			stringstream ss(line.erase(0, 2));	//删掉了"f "剩下的第一个就是数字
			for (int i = 0; i < 3; i++) {
				getline(ss, oneCorner, ' ');	//以单个空格为分隔符读取
